...
# Aby dodać własną muzykę, odkomentuj poniższe linie i umieść pliki w folderze gry:
# try:
#     MUSIC_EXPLORE = "audio/slowianski_mrok_ambience.ogg"
#     MUSIC_BATTLE = "audio/slowianska_szarza_combat.ogg"
# except Exception as e:
#     print("Brak plików audio, system używa trybu cichego/zastępczego.")
...
def play_dynamic_music(state_type):
    ...
    # Tutaj Pygame ładowałby odpowiednią ścieżkę:
    # if state_type == "BATTLE":
    #     pygame.mixer.music.load(MUSIC_BATTLE)
    #     pygame.mixer.music.play(-1)
    # else:
    #     pygame.mixer.music.load(MUSIC_EXPLORE)
    #     pygame.mixer.music.play(-1)
...
def draw_drozd(surface, x, y):
    ...
    # Długi płaszcz detektywa
    pygame.draw.rect(surface, (45, 50, 65), (x + 2, y + 14, 26, 22))
    pygame.draw.line(surface, (15, 15, 20), (x + 15, y + 14), (x + 15, y + 36), 2) # Szew płaszcza
    # Twarz i oczy
...
def draw_slavic_house(surface, x, y, width, height):
    ...
    # Drewniane ściany z grubych bali
...
def draw_monster_latarnik(surface, x, y, anim_tick):
    ...
    pygame.draw.circle(surface, (150, 0, 0), (x + 12, y - 1 + offset_y), 2) # Ślepia
    pygame.draw.circle(surface, (150, 0, 0), (x + 18, y - 1 + offset_y), 2)
    # Przeklęta czerwona latarnia
...
def draw_monster_mamuna(surface, x, y, anim_tick):
    ...
    # Włosy i zniekształcona sylwetka
...
def draw_monster_krzykacz(surface, x, y, anim_tick):
    ...
    # Wielka, czarna, krzycząca jama gębowa
    pygame.draw.circle(surface, (10, 5, 15), (x + 15, y + 22), int(8 * scale))
    # Świecące białe punkty zamiast oczu
...
# Drzewo Dialogowe i Wybory Fabularne (Rozgałęzienia)
dialogue_title = ""
dialogue_lines = []
dialogue_choices = []
...
while running:
    ...
    # ==========================================
    # 1. OBSŁUGA LOGIKI I INPUTU ZALEŻNIE OD STANU
    # ==========================================
    ...
    if current_state in [STATE_EXPLORE, STATE_HOUSE]:
        ...
        # Ograniczenia i interakcje na mapie głównej
        if current_state == STATE_EXPLORE:
            ...
            for m in monster_triggers:
                if not m["beaten"] and m["rect"].collidepoint(player_pos.x, player_pos.y):
                    ...
                    # Resetowanie i przygotowanie rzutu kośćmi
                    dice_timer = 0
            ...
            # Sprawdzanie wyjścia z domu
            in_any_house = False
            for h in houses:
                # Jeśli gracz wyjdzie poza obręb makiety domu, wraca na wieś
                if h.rect.inflate(40, 40).collidepoint(player_pos.x, player_pos.y):
                    in_any_house = True
                    # Wyzwalanie rozmowy z NPC przy zbliżeniu do środka izby
        ...
        # Sterowanie awatarem w trybie minigry (Szybkie uniki)
        c_speed = 5
        ...
        # --- LOGIKA UNIKALNYCH MINIGIER BOJOWYCH ---
        
        if active_boss_type == BOSS_LATARNIK:
            # Latarnik szarżuje horyzontalnie i strzela ognistymi kulami
            if combat_timer % 40 == 0:
                # Wystrzał pocisku kierowanego na gracza
            ...
            # Gracz zadaje obrażenia stałe zbliżając się do bossa lub automatycznie z bonusem kości
            if combat_timer % 60 == 0:
                boss_hp -= (base_attack + mod_attack)

        elif active_boss_type == BOSS_MAMUNA:
            # Hipnotyzujące kręgi rozszerzające się ze środka - musisz uciekać na krawędzie
            ...
            for p in combat_projectiles:
                if p.vx == 0 and p.vy == 0:
                    p.radius += 3 # Ekspansja fali hipnotycznej
            ...
            # Korzenie wybuchają losowo pod stopami gracza
            if combat_timer % 30 == 0:
                # Znacznik korzenia pobiera aktualną pozycję Drozda i po sekundzie uderza
            ...
            for p in combat_projectiles:
                if p.color == (139, 69, 19):
                    p.radius += 0.5
                    if p.radius >= 15: # Korzeń eksploduje z ziemi
            ...
            # Pełny balans: Krzykacz miota pierścieniami dźwiękowymi, a Gracz musi strzelać (Spacja)
        ...
        # Aktualizacja standardowych pocisków lecących
        for p in combat_projectiles:
            if p.vx != 0 or p.vy != 0:
                p.update()
    ...
    # ==========================================
    # 2. OBSŁUGA ZDARZEŃ KLAWIATURY (EVENT LOOP)
    # ==========================================
        ...
        if event.type == pygame.QUIT:
            ...
            if current_state == STATE_DIALOGUE:
                if event.key == pygame.K_w or event.key == pygame.K_UP:
                    ...
                    # Zatwierdzenie wyboru fabularnego (rozgałęzienie)
                    ...
                    current_state = STATE_HOUSE # powrót do wnętrza domu
    ...
    # ==========================================
    # 3. RENDEROWANIE GRAFIKI (WARSTWY)
    # ==========================================
    
    if current_state in [STATE_EXPLORE, STATE_DIALOGUE, STATE_DICE_ROLL]:
        # Rysowanie wygenerowanej makiety wsi
        ...
        # Rysowanie aktywnych potworów w ich leżach
        for m in monster_triggers:
            if not m["beaten"]:
        ...
        # Rysowanie postaci Drozda
        draw_drozd(screen, int(player_pos.x) - 15, int(player_pos.y) - 20)

    elif current_state == STATE_HOUSE:
        # Wnętrze chaty
        screen.fill((25, 20, 15)) # podłoga z ciemnych desek
        pygame.draw.rect(screen, (45, 35, 25), (50, 50, WIDTH-100, HEIGHT-100), 8) # ściany izby
        
        # Stół na środku izby
        pygame.draw.rect(screen, (70, 50, 35), (WIDTH//2 - 40, HEIGHT//2 - 20, 80, 50))
        
        # Postać NPC rezydująca w domu
        ...
        # ARENA MINIGRY BOJOWEJ
        screen.fill((10, 12, 18))
        pygame.draw.rect(screen, (150, 30, 30), (80, 100, WIDTH-160, HEIGHT-160), 3) # Granice areny
        
        # Interfejs życia i modyfikatorów kości
        ...
        # Renderowanie konkretnego potwora w centrum areny
